  ...
  bool EncoderFeatureIndex::buildFeature(LearnerPath *path)
  {
...

    {
      ...
      else
      {
        ...
        feature_cache_.insert(std::pair<std::string, std::pair<const int *, size_t>>(key,
                                                                                     std::pair<const int *, size_t>(path->rnode->fvector, 1)));
      }
    }

    {
      ...
      else
      {
        if (!buildBigramFeature(path, rfeature1.c_str(), lfeature2.c_str()))
          return false;
        feature_cache_.insert(std::pair<std::string, std::pair<const int *, size_t>>(key, std::pair<const int *, size_t>(path->fvector, 1)));
      }
    }
